...
export default function Message({ msg, isLatestAgentMsg, isThinking, onAction, onAddToCart }) {
  const isAgent = msg.role === 'agent';
  const [activeModalItem, setActiveModalItem] = useState(null);
  const [modalType, setModalType] = useState('food');

  // Track orchestration timeline animation for avatar thinking state
  const [isTimelineAnimating, setIsTimelineAnimating] = useState(Boolean(isAgent && isLatestAgentMsg && msg.rankings && msg.rankings.length > 0));

  useEffect(() => {
    if (isTimelineAnimating) {
      const t = setTimeout(() => setIsTimelineAnimating(false), 3200);
      return () => clearTimeout(t);
    }
  }, [isTimelineAnimating]);

  const isAvatarThinking = Boolean(isThinking || isTimelineAnimating);
  ...
  return (
    <div className="message-row agent">
      <div className="agent-avatar-wrap">
        <div 
          className={`agent-avatar-circle ${isAvatarThinking ? 'thinking' : 'idle'}`}
          title={isAvatarThinking ? 'Agent thinking...' : 'Swiggy MCP Agent'}
        >
          <img 
            src="/swiggy_avatar.png" 
            alt="Swiggy AI" 
      ...
      <div className="message-bubble">
        {isLatestAgentMsg && msg.rankings && msg.rankings.length > 0 && (
          <>
            <OrchestrationTimeline 
              ...
              query={msg.content} 
            />
            {/* Memory Signal Caption: show when winner is strongly dominant */}
            {(() => {
              const sorted = [...msg.rankings].sort((a, b) => b[1] - a[1]);
              if (sorted.length >= 2 && sorted[0][1] >= sorted[1][1] * 1.5) {
                const winnerName = sorted[0][0].charAt(0).toUpperCase() + sorted[0][0].slice(1);
                return (
                  <div className="memory-signal-caption">
                    ⚡ Strongly favored <strong>{winnerName}</strong> for this one.
                  </div>
                );
              }
              return null;
