...
export default function OrchestrationTimeline({ activeServer, rankings, query }) {
  ...
  // Get raw score (0.0–2.0) for confidence bar width
  const getRawScore = (serverName) => {
    if (rankings) {
      const r = rankings.find(x => x[0] === serverName);
      if (r) return Math.min(2.0, r[1]);
    }
    return serverName === activeServer ? 1.85 : (serverName === 'instamart' ? 0.52 : 0.33);
  };
        ...
        <div className="routing-container">
          ...
          <div style={{ border: '1px solid var(--border-color)', borderRadius: '6px', padding: '8px' }}>
            {servers.map(s => {
              ...
              const rawScore = getRawScore(s.name);
              const barWidth = Math.round((rawScore / 2.0) * 100);
              return (
                <div key={s.name} className={`routing-server-row ${isSelected ? 'selected' : ''}`} style={{ position: 'relative', overflow: 'hidden' }}>
                  {/* Confidence bar */}
                  <div style={{
                    position: 'absolute',
                    left: 0,
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: '8px', position: 'relative', zIndex: 1 }}>
                    <span style={{ fontWeight: isSelected ? 800 : 500 }}>{getScore(s.name)}</span>
                    {isSelected && <span style={{ fontSize: '10px', color: 'var(--orange-primary)' }}>&larr; SELECTED</span>}
                  </span>
                </div>
              );
            })}
